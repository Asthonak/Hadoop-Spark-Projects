...
//    val noPunctuation = lines.map(x => x.replaceAll("[,.!?:;]",""))
    ...
    // replace all "RT @" with "RT@" (will make processing easier)
    val RTCombine = lines.map(x => x.replaceAll("RT @","RT@"))

    // break down lines into individual words
    // use "distinct" so that mentions are only added once for each line
    val words = RTCombine.flatMap(line=>line.split("\\s+")/*.distinct*/)

    // filter words to only include mentions
    val retweets = words.filter(x => x.startsWith("RT@") == true)

    // re-sepereate the RT's from the @'s
    val RTSeparate = retweets.map(x => x.replaceAll("RT@","RT @"))

    // count how many times each mention is referenced
    val count = RTSeparate.map(x => (x, 1)).reduceByKey( (x,y) => x + y )

    // sort mentions in descending order 
    // top(10) makes it show only the first 20 results
    val sortedCount = count.map( x => (x._2, x._1) ).sortByKey(false,1).top(10)
